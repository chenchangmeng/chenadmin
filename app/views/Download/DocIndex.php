		...
		<div class="col-xs-10 column">	
			...
			<div class="row clearfix">
				<div class="col-xs-12 column">
					...
					<?php echo Form::open(array('url' => '', 'method' => 'post',  'id'=>'doc_add_form', 'name'=>'doc_add_form'));  ?>
					    <input type="hidden" id="docBasicId" name="docBasicId" value="<?php echo $docBasicData[0]->id; ?>" />
					    <script id="docInfo" name="docInfo" type="text/plain">
					        <?php echo $docBasicData[0]->content; ?>
					    </script>
					    <!-- 配置文件 -->
				...
				<div class="col-xs-4 column" style="padding-left:102px;padding-top:10px;">
					...
					<a href="<?php echo URL::to('download/doc-add'); ?>"  class="btn btn-primary btn-custom">
						<em class='glyphicon glyphicon-plus'></em>添加文档
					</a>
				</div>
			</div>
			...
			<div id="list_content_div">
				<table class="table">
					...
					<tbody>
						<?php $i = 0; ?>
						<?php foreach($docData as $value){ ?>
							<?php echo $i == 1 ? "<tr class='active'>" : "<tr>"; ?>	
								<td><?php echo $value->docName;  ?></td>
								<td><?php echo $value->docTypeName;  ?></td>
								<td><?php echo $value->created_at; ?></td>
								<td>
									<a href="<?php echo URL::to('download/doc-update/'.$value->id); ?>"><em class="glyphicon glyphicon-edit"></em>编辑</a>/
									<a href="javascript:void(0);" onclick="DeleteDoc(<?php echo $value->id; ?>)"><em class='glyphicon glyphicon-remove'></em>删除</a>
								</td>
...
pagelist.mUrl = "<?php echo URL::to('download/doc-page'); ?>";
...
function saveInfo(){
	var  ll = document.getElementById("ueditor_textarea_docInfo");

	if(ll){
		var v = ll.value;
		var id = document.getElementById("docBasicId").value;
		if(v.length > 0){
...
function DeleteDoc(id){
	if(confirm("确定要删除该手册吗？")){
		window.location.href = "<?php echo URL::to('download/doc-delete/'); ?>/" + id;
	}
}
